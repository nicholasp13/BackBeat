...
namespace BackBeat {
	...
	{
	public:
		SynthProcessor(AudioProps props, std::shared_ptr<SynthEngine> engine, UUID id);
		~SynthProcessor();

		virtual void ProcessSamples(unsigned int numFrames, unsigned int sampleRate, unsigned int numChannels) override;
