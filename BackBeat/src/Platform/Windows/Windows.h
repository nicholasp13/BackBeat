...
namespace BackBeat {
	...
	namespace Windows {
		...
		static bool CheckFailure(long result)
		{
			// HRESULT ERROR CODE CHECK (from Windows API regarding capturing and rendering audio streams)
			{
...
#define CHECK_FAILURE( result ) if (!BackBeat::Windows::CheckFailure(result)) goto FailureExit;
